...
namespace BookStoreApp.API.Controllers
{
    ...
    public class AuthorsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<AuthorReadOnlyDto>>> GetAuthors()
        {
            try
            {
                var authors = _mapper.Map<IEnumerable<AuthorReadOnlyDto>>(
        ...
        public async Task<ActionResult<AuthorReadOnlyDto>> GetAuthor(int id)
        {
            ...
            {
                _logger.LogError(ex, $"Errores Performing GET in {nameof(GetAuthor)}");
                return StatusCode(500, Messages.Error500Message);
            }
        }
        ...
        public async Task<IActionResult> PutAuthor(int id, AuthorUpdateDto authorDto)
        {
            ...
            if (author == null)
            {
                _logger.LogWarning($"{nameof(Author)} record not found in {nameof(PutAuthor)}");
                return NotFound();
            }
        ...
        public async Task<IActionResult> DeleteAuthor(int id)
        {
            try
            {
                ...
                if (author == null)
                {
                    _logger.LogWarning($"{nameof(Author)} record not found in {nameof(DeleteAuthor)}");
                    return NotFound();
                }
